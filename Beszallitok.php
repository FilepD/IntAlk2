...
<body>
    <div class="container-fluid">
        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post">
            <div class="form-row">
                <div class="col-1">Beszállítók</div>
                <div class="col-2">
                ...
                <div class="col-2">
                    ...
                    if ($_SERVER["REQUEST_METHOD"] == "POST") {
                        $sql = "SELECT MAX(ID) FROM beszallito";
                        ...
                        $sqlInsert = "INSERT INTO beszallito (ID, Név, Cim, Telefon) VALUES (?,?,?,?)";
                        $Nev =  $_POST["InputNev"];
                        $Cim =  $_POST["InputCim"];
            ...
            <tbody>
                ...
                if (mysqli_num_rows($records) > 0) {
                    while ($row = mysqli_fetch_assoc($records)) {
                        ...
                        '<td>', $row["Cim"], '</td>',
                        '<td>', $row["Telefon"], '</td>',
                        '<td><a href="UpdateBeszForm.php?id=' . $row["ID"] . '" class="btn btn-primary btn-sm max">Módosítás</a></td>',
                        '<td><a href="deleteBesz.php?id=' . $row["ID"] . '" class="btn btn-danger btn-sm max">Törlés</a></td>',
                        '</tr>';
                    }
                } else {
                    echo "0 results ";
